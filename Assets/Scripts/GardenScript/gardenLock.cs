...
public class gardenLock : MonoBehaviour
{
    ...
    public bool isTouch;  //si l'écran est touché par le joueur 
    public float angle;
    ...
    void Start()
    {
        ...
        screenText.text = "BADGE needed or enter emergency code :";
    ...
    void Update()
    {
        ...
        // Si on touche l'écran, alors la roue de choix du code apparaît
        if (isTouch)
        {
            if (initWheelOK)
            {
    ...
    public void addNumber(int id)
    {
        ...
        if (tryCode.text.Length < 5 && unChiffreEnPlusPasPlus)
        {
    ...
    public void initiateButtonWheel()
    {
        foreach (GameObject i in gObjList)
        {
            ...
            bouton.transform.localPosition = new Vector3(rayonBoutons * Mathf.Cos(Mathf.Deg2Rad * gObjList.IndexOf(i) * espacementBoutons),rayonBoutons * Mathf.Sin(Mathf.Deg2Rad * gObjList.IndexOf(i) * espacementBoutons), 0.1f);
            bouton.transform.eulerAngles = new Vector3(bouton.transform.eulerAngles.x + 90f, bouton.transform.eulerAngles.y, bouton.transform.eulerAngles.z);
            bouton.transform.localScale = bouton.transform.localScale *2;
